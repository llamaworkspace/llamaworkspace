...
export const SettingsMembersTableRoleSelector = ({
  ...
  const { mutate: updateUserRoleForWorkspace } = useUpdateUserRoleForWorkspace()

  // When it's an inviteId (not yet a user), we don't have an id
  // FIXME, revert !isOwner to isOwner
  const isDisabled = !userId || !isOwner || value !== UserRole.Admin.toString()

  const [finalValue, setFinalVaue] = useState(
    value || UserRole.Member.toString(),
  )
  const handleValueChange = (nextValue: string) => {
    setFinalVaue(nextValue)
    console.log('handleValueChange', nextValue, userId)
  }
